...
public class MomentumDamageOverlay implements LayeredDraw.Layer {
    private static MomentumDamageOverlay instance;

    public static MomentumDamageOverlay getInstance() {
        if (instance == null) {
            instance = new MomentumDamageOverlay();
        }
        return instance;
    }

    public final static ResourceLocation TEXTURE = ResourceLocation.fromNamespaceAndPath(CqsArmory.MODID, "textures/gui/damage_stacks_bar.png");
    ...
    static final int TEXT_COLOR = ChatFormatting.DARK_GREEN.getColor();
    ...
    public void render(GuiGraphics guiHelper, DeltaTracker deltaTracker) {
        ...
        if (!shouldShowMomentumDamage(player))
            return;

        int maxStacks = 10; // TBD FIXME
        int stacks = (int) AbilityData.get(player).momentumOrbEffects.arrowDamageStacks;
        int barX, barY;
        int configOffsetY = com.example.cqsarmory.config.ClientConfigs.MOMENTUM_DAMAGE_Y_OFFSET.get();
        int configOffsetX = com.example.cqsarmory.config.ClientConfigs.MOMENTUM_DAMAGE_X_OFFSET.get();
        Anchor anchor = com.example.cqsarmory.config.ClientConfigs.MOMENTUM_DAMAGE_ANCHOR.get();
        ...
        textX = com.example.cqsarmory.config.ClientConfigs.MOMENTUM_DAMAGE_TEXT_X_OFFSET.get() + barX + imageWidth / 2 - (int) ((("" + stacks).length() + 0.5) * CHAR_WIDTH);
        textY = com.example.cqsarmory.config.ClientConfigs.MOMENTUM_DAMAGE_TEXT_Y_OFFSET.get() + barY + (anchor == Anchor.XP ? ICON_ROW_HEIGHT / 3 : ICON_ROW_HEIGHT);

        int timerTextX, timerTextY;
        String stacksTimer = ((AbilityData.get(player).momentumOrbEffects.arrowDamageEnd - player.tickCount) / 20 ) + "s";
    ...
    public static boolean shouldShowMomentumDamage(Player player) {
        //We show momentum damage if their momentum damage is not 0
        var display = com.example.cqsarmory.config.ClientConfigs.MOMENTUM_DAMAGE_DISPLAY.get();
        return !player.isSpectator() && display != Display.Never &&
                (display == Display.Always || AbilityData.get(player).momentumOrbEffects.arrowDamageStacks != 0);
    ...
    private static int getBarY(Anchor anchor, int screenHeight, Gui gui, Player player) {
        ...
        if (anchor == Anchor.TopLeft || anchor == Anchor.TopRight)
            return SCREEN_BORDER_MARGIN;
        if (AbilityData.get(player).momentumOrbEffects.speedStacks > 0)
            return screenHeight - SCREEN_BORDER_MARGIN - IMAGE_HEIGHT - 25;

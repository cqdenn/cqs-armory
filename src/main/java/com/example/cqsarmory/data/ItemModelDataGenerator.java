...
public class ItemModelDataGenerator extends ModelProvider<ItemModelBuilder> {
    ...
    public ItemModelBuilder ingotItem(DeferredHolder<Item, ? extends Item> item) {
        return ((AtlasBuilder) (withExistingParent(item.getId().getPath(),
                ResourceLocation.withDefaultNamespace("item/generated")))).handler(CqsArmory.MODID + ":weaponset_handler").loader("atlas_api:dynamic_model");
    }
    ...
    public ItemModelBuilder crossbowItemStandalone(DeferredHolder<Item, ? extends Item> item) {
        return ((AtlasBuilder) (withExistingParent(item.getId().getPath(),
                ResourceLocation.fromNamespaceAndPath(CqsArmory.MODID, "item/template_crossbow"))))
                .handler(CqsArmory.MODID + ":weaponset_handler")
                .loader("atlas_api:simple_model")
                .texture("layer0",
                        ResourceLocation.fromNamespaceAndPath(CqsArmory.MODID, WeaponsetAssetHandler.getCrossbowAtlasLocation((CrossbowItem) item.get()))
                )
                .override().predicate(ResourceLocation.withDefaultNamespace("pulling"), 1).model(makeCrossbowPullingStandalone(item, 0)).end()
                .override().predicate(ResourceLocation.withDefaultNamespace("pulling"), 1).predicate(ResourceLocation.withDefaultNamespace("pull"), 0.58f).model(makeCrossbowPullingStandalone(item, 1)).end()
                .override().predicate(ResourceLocation.withDefaultNamespace("pulling"), 1).predicate(ResourceLocation.withDefaultNamespace("pull"), 1f).model(makeCrossbowPullingStandalone(item, 2)).end()
                .override().predicate(ResourceLocation.withDefaultNamespace("charged"), 1).model(makeCrossbowChargedStandalone(item, true)).end()
                .override().predicate(ResourceLocation.withDefaultNamespace("charged"), 1).predicate(ResourceLocation.withDefaultNamespace("firework"), 1).model(makeCrossbowChargedStandalone(item, false)).end();

    }

    public ItemModelBuilder makeCrossbowChargedStandalone(DeferredHolder<Item, ? extends Item> item, boolean arrow) {
        String template = arrow ? "_arrow" : "_firework";
        return ((AtlasBuilder) (withExistingParent(item.getId().getPath() + template,
                ResourceLocation.fromNamespaceAndPath(CqsArmory.MODID, "item/template_crossbow"))))
                .handler(CqsArmory.MODID + ":weaponset_handler")
                .loader("atlas_api:simple_model")
                .texture("layer0",
                        ResourceLocation.fromNamespaceAndPath(CqsArmory.MODID, WeaponsetAssetHandler.getChargedCrossbowAtlasLocation((CrossbowItem) item.get(), arrow))
                );
    }

    public ItemModelBuilder makeCrossbowPullingStandalone(DeferredHolder<Item, ? extends Item> item, int pull) {
        return ((AtlasBuilder) (withExistingParent(item.getId().getPath() + "_pulling_" + pull,
                ResourceLocation.fromNamespaceAndPath(CqsArmory.MODID, "item/template_crossbow"))))
                .handler(CqsArmory.MODID + ":weaponset_handler")
                .loader("atlas_api:simple_model")
                .texture("layer0",
                        ResourceLocation.fromNamespaceAndPath(CqsArmory.MODID, WeaponsetAssetHandler.getCrossbowAtlasLocation((CrossbowItem) item.get(), pull))
                );
    }
